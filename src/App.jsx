...
export default function App() {
  ...
  // --- Dynamic Styling mapping for Night/Day modes ---
    ...
    <div className={`min-h-screen ${bgMain} ${textMain} font-sans p-4 md:p-6 lg:p-8 selection:bg-blue-100 transition-colors duration-300`}>
      ...
      <div className="max-w-[1400px] mx-auto space-y-6">
        ...
        <header className={`${cardStyle} p-4 pr-6 flex items-center justify-between z-20`}>
          ...
          <div className="flex items-center space-x-2 md:space-x-3">
              ...
              <Tooltip text={ratesStatus === 'live' ? 'Live Exchange Rates Active' : 'Offline Fallback Rates'}>
                <div className={`flex items-center ${isDarkMode ? 'bg-[#0F172A] border-slate-700' : 'bg-slate-100 border-slate-200/50'} p-1 rounded-2xl border relative transition-colors`}>
                  ...
                  {Object.keys(currencyConfig).map((curr) => (
                    <button key={curr} onClick={() => handleCurrencyChange(curr)} className={`px-3 py-1.5 text-xs font-bold rounded-xl transition-all ${currency === curr ? (isDarkMode ? 'bg-[#1E293B] text-blue-400 shadow-sm' : 'bg-white text-blue-600 shadow-sm') : (isDarkMode ? 'text-slate-400 hover:text-slate-200' : 'text-slate-500 hover:text-slate-800')}`}>
                      {curr}
                    </button>
                  ))}
                </div>
            ...
            <div className={`flex items-center space-x-1 ${isDarkMode ? 'bg-[#0F172A] border-slate-700/80' : 'bg-slate-100 border-transparent'} p-1 rounded-[20px] border`}>
               <Tooltip text={isDarkMode ? "Switch to Day Mode" : "Switch to Night Mode"}>
                 <button onClick={() => setIsDarkMode(!isDarkMode)} className={`flex items-center text-sm font-bold ${isDarkMode ? 'text-amber-400 hover:bg-[#1E293B]' : 'text-slate-600 hover:text-amber-600 hover:bg-white shadow-sm'} p-2.5 rounded-[14px] transition-all`}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 z-10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <div className="md:col-span-2">
                    <label className={`flex items-center text-sm font-bold ${textMain} mb-2`}>
                      Automation Name 
                      <Tooltip text="Give your automation project a short, recognizable name."><Info size={14} className={`${textSub} hover:text-blue-500 transition-colors cursor-help`}/></Tooltip>
                    </label>
                  ...
                  <div className="md:col-span-2">
                    <label className={`flex items-center text-sm font-bold ${textMain} mb-2`}>
                      ...
                      <Tooltip text='What specific manual process is this bot replacing? (e.g., "Resetting user passwords")'><Info size={14} className={`${textSub} hover:text-blue-500 transition-colors cursor-help`}/></Tooltip>
                    </label>
                  ...
                  <div className="md:col-span-2">
                    <label className={`flex items-center text-sm font-bold ${textMain} mb-2`}>
                      ...
                      <Tooltip text='Which business metrics will this improve? (e.g., "Average Handle Time", "Error Rate")'><Info size={14} className={`${textSub} hover:text-blue-500 transition-colors cursor-help`}/></Tooltip>
                    </label>
                  ...
                  <div>
                    <label className={`flex items-center text-sm font-bold ${textMain} mb-2`}>
                      ...
                      <Tooltip text='What pain points are solved? (e.g., "Too many typos", "Slow response time")'><Info size={14} className={`${textSub} hover:text-blue-500 transition-colors cursor-help`}/></Tooltip>
                    </label>
                  ...
                  <div>
                    <label className={`flex items-center text-sm font-bold ${textMain} mb-2`}>
                      ...
                      <Tooltip text='What are the soft benefits? (e.g., "Higher employee morale", "Better compliance")'><Info size={14} className={`${textSub} hover:text-blue-500 transition-colors cursor-help`}/></Tooltip>
                    </label>
              ...
              <div className={`p-6 md:p-8 grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-7 ${panelBg} rounded-b-[28px]`}>
                ...
                <div>
                  ...
                  <div className="flex space-x-3">
                    ...
                    <div className="relative flex-1">
                      <input type="number" step="0.01" value={effortHours !== '' ? Number(effortHours).toFixed(2) : ''} onChange={(e) => setEffortHours(e.target.value)} placeholder="0.00" className={`${effortHours !== '' && effortHours < 0 ? inputErrorStyle : inputStyle} pr-12 font-mono text-lg ${isDarkMode ? 'bg-[#0F172A]' : 'bg-slate-100/50'}`} />
                      <span className={`absolute inset-y-0 right-0 pr-4 flex items-center ${textSub} font-bold text-xs pointer-events-none`}>HR</span>
                    </div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    <div className={`absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none ${textSub}`}><Coins size={18} /></div>
                ...
                <div>
                  ...
                  <div className="relative">
                    <div className={`absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none ${textSub}`}><Briefcase size={18} /></div>
                    ...
                    <span className={`absolute inset-y-0 right-0 pr-4 flex items-center ${textSub} font-bold text-xs pointer-events-none`}>MONTHS</span>
                  </div>
                </div>
                ...
                <div className={`md:col-span-2 pt-5 border-t ${borderMuted} mt-2`}>
                  <div className="flex items-center space-x-2 mb-4">
                    ...
                    <h3 className={`text-sm font-bold ${textHeading} uppercase tracking-wider`}>Investment & Ongoing Costs</h3>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <div className={`${isDarkMode ? 'bg-blue-950/20 border-blue-900/40' : 'bg-blue-50/50 border-blue-100/50'} p-4 rounded-2xl border flex flex-col justify-between`}>
                      <div>
                        ...
                        <p className={`text-[10px] ${textSub} mb-3 leading-tight`}>First 12 mo support cost.</p>
                      </div>
                      <div className="relative">
                        <div className={`absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none ${textSub}`}><Users size={14} /></div>
                        <input type="number" min="0" value={sreCostY1} onChange={(e) => setSreCostY1(e.target.value)} placeholder="0" className={`w-full px-3 py-2 ${isDarkMode ? 'bg-[#0F172A] border-slate-700 text-slate-200' : 'bg-white border-slate-200 text-slate-800'} border rounded-xl focus:ring-2 focus:ring-blue-500 outline-none text-sm font-mono pl-8 transition-colors`} />
                      </div>
                    </div>
                    ...
                    <div className={`${isDarkMode ? 'bg-emerald-950/20 border-emerald-900/40' : 'bg-emerald-50/50 border-emerald-100/50'} p-4 rounded-2xl border flex flex-col justify-between`}>
                      <div>
                        ...
                        <p className={`text-[10px] ${textSub} mb-3 leading-tight`}>Ongoing tapered support.</p>
                      </div>
                      <div className="relative">
                        <div className={`absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none ${textSub}`}><Users size={14} /></div>
                        <input type="number" min="0" value={sreCostY2} onChange={(e) => setSreCostY2(e.target.value)} placeholder="0" className={`w-full px-3 py-2 ${isDarkMode ? 'bg-[#0F172A] border-slate-700 text-slate-200' : 'bg-white border-slate-200 text-slate-800'} border rounded-xl focus:ring-2 focus:ring-blue-500 outline-none text-sm font-mono pl-8 transition-colors`} />
                      </div>
                    </div>
          ...
          <div className="lg:col-span-5 space-y-6">
            ...
            <div className={`flex flex-col sm:flex-row sm:items-center justify-between gap-3 ${isDarkMode ? 'bg-[#1E293B] border-slate-700/60' : 'bg-white border-slate-200/60'} p-2 pl-5 rounded-3xl border shadow-sm`}>
              <h3 className={`text-sm font-bold ${textHeading} flex items-center`}>
                <Activity size={16} className="mr-2 text-blue-500" /> Forecast Scenario
              </h3>
            ...
            <div className={`${cardStyle} flex flex-col`}>
              <div className="p-6 md:p-8 flex-1">
                <h3 className={`text-base font-bold ${textHeading} mb-6 flex items-center`}>
                  <ArrowRightLeft size={18} className="mr-2 text-blue-500"/> Current vs. Future State
                </h3>
                ...
                <div className="space-y-6">
                  ...
                  <div className={`${isDarkMode ? 'bg-[#0F172A]' : 'bg-slate-50'} p-5 rounded-[20px] border ${borderMuted} shadow-[inset_0_1px_3px_rgba(0,0,0,0.02)]`}>
                    ...
                    <div className="flex justify-between items-start mb-4">
                      <div>
                         <div className={`text-[11px] font-bold ${textSub} uppercase mb-1`}>Monthly Labor Cost</div>
                         <div className={`text-xl font-extrabold ${textHeading} tracking-tight`}>{formatCurrency(results.currentMonthlyCost)}</div>
                      </div>
                      <div className="text-right">
                         <div className={`text-[11px] font-bold ${textSub} uppercase mb-1`}>Manual Effort</div>
                         <div className={`text-xl font-extrabold ${textHeading} tracking-tight`}>
                           {new Intl.NumberFormat().format(Math.round(results.totalManualHoursMonthly))} <span className={`text-sm font-medium ${textSub}`}>hrs/mo</span>
                         </div>
                         <div className={`text-xs font-semibold ${textSub} mt-1 ${isDarkMode ? 'bg-slate-800' : 'bg-slate-200/50'} inline-block px-2 py-0.5 rounded-md`}>({results.currentFte.toFixed(1)} FTEs)</div>
                      </div>
                    </div>
                    
                    {challenges && (
                      <div className={`border-t ${borderMuted} pt-3`}>
                        ...
                        <p className={`text-xs ${textMain} leading-relaxed whitespace-pre-wrap font-medium`}>{challenges}</p>
                      </div>
                    )}
                  </div>
                  ...
                  <div className={`${isDarkMode ? 'bg-blue-950/20 border-blue-900/40' : 'bg-blue-50/50 border-blue-100/60'} p-5 rounded-[20px] border shadow-[inset_0_1px_3px_rgba(59,130,246,0.05)]`}>
                    ...
                    <div className="flex justify-between items-start mb-4">
                      <div>
                         ...
                         <div className={`text-xl font-extrabold ${isDarkMode ? 'text-blue-100' : 'text-blue-900'} tracking-tight`}>{formatCurrency(results.futureMonthlyCostAvg)}</div>
                      </div>
                      <div className="text-right">
                         <div className="text-[11px] font-bold text-blue-400 uppercase mb-1">Residual Effort</div>
                         <div className={`text-xl font-extrabold ${isDarkMode ? 'text-blue-100' : 'text-blue-900'} tracking-tight`}>
                           {new Intl.NumberFormat().format(Math.round(results.remainingManualHoursMonthly))} <span className="text-sm font-medium text-blue-500">hrs/mo</span>
                         </div>
                         <div className={`text-xs font-semibold text-blue-500 mt-1 ${isDarkMode ? 'bg-blue-950/50' : 'bg-blue-100/50'} inline-block px-2 py-0.5 rounded-md`}>({results.toBeFte.toFixed(1)} FTEs)</div>
                      </div>
                    </div>
              ...
              <div className="p-6 md:p-8 pt-0 mt-auto">
                <div className={`border-t ${borderMuted} pt-5`}>
                  <button onClick={generateROIInsights} disabled={isGeneratingInsights} className={`w-full flex items-center justify-center space-x-2 text-sm font-bold py-3.5 rounded-2xl transition-colors disabled:opacity-70 disabled:cursor-not-allowed shadow-sm border ${isDarkMode ? 'bg-blue-950/30 hover:bg-blue-900/50 text-blue-400 border-blue-900/50' : 'text-blue-700 bg-blue-50 hover:bg-blue-100 border-blue-100'}`}>
                  ...
                  {roiInsights && (
                    <div className={`mt-4 ${isDarkMode ? 'bg-indigo-950/20 border-indigo-900/30 text-indigo-200' : 'bg-indigo-50/80 border-indigo-100/50 text-indigo-900'} p-5 rounded-2xl text-sm font-medium leading-relaxed border overflow-y-auto max-h-40 shadow-inner custom-scrollbar whitespace-pre-wrap`}>
                      {roiInsights}
                    </div>
                  )}
                </div>
        ...
        <div className={`${cardStyle} mt-6 mb-12`}>
          <button onClick={() => setIsHowItWorksOpen(!isHowItWorksOpen)} aria-expanded={isHowItWorksOpen} aria-controls="how-it-works-content" className={`w-full p-6 md:p-8 flex items-center justify-between ${isDarkMode ? 'hover:bg-slate-800/50' : 'hover:bg-slate-50'} transition-colors text-left outline-none rounded-[28px]`}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-6">
                <div className="space-y-6">
                  ...
                  <div>
                    ...
                    <p className={`text-sm ${textSub} leading-relaxed font-medium mt-2`}>Scores over 80 are considered strong investments. You can toggle visibility using the eye icon in the results panel.</p>
                  </div>
